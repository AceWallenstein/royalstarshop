...
public class GoodsVideoDetailActivity extends BaseActivity {
    ...
    private HomeModel.DataBean.VideoGoodsBean dataBeanList = new HomeModel.DataBean.VideoGoodsBean();
    private int time;
    private int pos;
            ...
            if (videoView.isPlaying()) {
                ...
                if (pos == 0) {
                    if (time == 0) {
                        ...
                        String videoUrl2 = dataBeanList.getVideo().getFile_path();
                        //设置视频控制器
...
//                                    // video 视屏播放的时候把背景设置为透明
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dataBeanList = (HomeModel.DataBean.VideoGoodsBean) getIntent().getSerializableExtra("dataBeanList");
        Glide.with(this).load(dataBeanList.getGoods_image()).into(ivThumb);
        Glide.with(this).load(dataBeanList.getGoods_image()).apply(bitmapTransform(new GlideRoundTransform(this))).into(ivImage);
        tvName.setText(dataBeanList.getGoods_name());
        tvMoney.setText(dataBeanList.getGoods_sku().getGoods_price());
        tvSales.setText(dataBeanList.getGoods_sku().getGoods_sales() + "件已售");
        tvVipPrice.setText("会员￥" + dataBeanList.getGoods_sku().getBalance_price());
